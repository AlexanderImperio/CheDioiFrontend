var myControllerProfissaoRead = angular.module("myControllerProfissaoRead", []);

myControllerProfissaoRead.controller('Read', ['$rootScope', '$scope', '$http', '$location',
    async function ($rootScope, $scope, $http, $location) {
        $rootScope.profissoes;

        if (!$rootScope.profissoes) {
            $rootScope.profissoes = [
        ...
        $scope.Deletar = function (Profissao) {
            if (confirm('Deseja realmente Deletar ' + Profissao.Descricao + '?')) {
                $rootScope.profissoes.map((value, index) => {
                    if (value.IdProfissao == Profissao.IdProfissao) {
                        $rootScope.profissoes.splice(index, 1);
                        alert(Profissao.Descricao + ' deletado com sucesso!');
                    }
                });
            }
        }
